 ...
 *
 */
public class CustomizeEmailableReport implements IReporter {  
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
    		 String dirTemp) {
    	String outdir = Constants.CUSTOMIZE_EMAILABLE_REPORT_FOLDER;
        try {
        ...
        generateResultSummaryReport(suites);
    ...
    protected void generateMethodSummaryReport(List<ISuite> suites) {
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r2 : r.values()) {
                ...
                resultSummaryWithoutException(suite, testContext.getFailedConfigurations(),
                        testName, "failed", " (configuration methods)");
                resultSummaryWithoutException(suite, testContext.getFailedTests(), testName,
                        "failed", "");
                resultSummaryWithoutException(suite, testContext.getSkippedConfigurations(),
                        testName, "skipped", " (configuration methods)");
                resultSummaryWithoutException(suite, testContext.getSkippedTests(), testName,
                        "skipped", "");
/*                resultSummaryWithoutException(suite, testContext.getPassedTests(), testName,
                        "passed", "");*/
                testIndex++;
            }
        }
     ...
     * the method details
     */
    protected void generateResultSummaryReport(List<ISuite> suites) {
        methodIndex = 0;
        startResultTable("methodOverview");
        int testIndex = 1;
        for (ISuite suite : suites) {
          if(suites.size()>1) {
            titleRow(suite.getName(), 5);
      ...
      if (tests.getAllResults().size() > 0) {
        ...
        for (ITestNGMethod method : getMethodSet(tests, suite)) {
          ...
          if (mq == 0) {
            ...
            titleResultRow(testname + " &#8212; " + style + details, 5, id);
            testIndex = null;
          }
          ...
          buff.append("<td><a href=\"#m" + methodIndex + "\">"
             ...
             // + "<td class=\"numi\">" + (end - start) + "</td>"
              ...
              + "<td class=\"numi\">"+ millisToTimeConversion(end - start) + "</td>"
              + "</tr>");
        }
        if (mq > 0) {
          cq += 1;
   ...
   * @param tests
   */
  private void resultSummaryWithoutException(ISuite suite, IResultMap tests, String testname,
...
/*                          for (String line : msgs) { 
                    	  ...
                    	  int startLine = msgs.size()-2;
                          for (int i = startLine; i<msgs.size();i++) {
                        	  // only detail for FAILED test cases with last info and screen-shot.
                        	  screenshotLnk+=msgs.get(i);
                        	  if(mq>=10) {
                        		  // if more than 10 test cases are failed, "custom-emailable-report.html" will display screen-shot only for first 10 failed test cases.
                        		  break;
                        	  }
                          }
                      }
...
//                      + "<td class=\"numi\" style=\"text-align:left;padding-right:2em\">" + firstLine+"<br/>"+screenshotLnk+ "</td>"
                      + "<td class=\"numi\" style=\"text-align:left;padding-right:2em\">" +screenshotLnk+ "</td>"
          ...
          if(mq>10) {
        	  out.print("<tr>***Screenshots displayed only for 10 first failed tests</tr>");
          }
      }
  }
//    /**
...
//    }
    ...
    private void startResultSummaryTable(String style) {
        ...
        out.println("<tr><th>Class</th>"
                + "<th>Method</th><th>Exception</th><th>Start Time </th><th>Time<br/>(hh:mm:ss)</th></tr>");
        row = 0;
    }   
    
    /** Starts and defines columns result summary table */
    private void startResultTable(String style) {
    	tableStart(style, "summary");
    	out.println("<tr><th>Class</th>"
...
//        tableColumnStart("Excluded<br/>Groups");
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r : tests.values()) {
                ...
                startTime = summaryFormat.format(overview.getStartDate()).toString();
                endTime = summaryFormat.format(overview.getEndDate()).toString();
                totalTime = millisToTimeConversion((overview.getEndDate().getTime() - overview
                        .getStartDate().getTime())/1000);
//                summaryCell(overview.getIncludedGroups());
//                summaryCell(overview.getExcludedGroups());
                out.println("</tr>");
                testIndex++;
                ExtentTestManager.reportTotalTime(startTime,
                		endTime,totalTime); 
            }
        }
        if (qty_tests > 1) {
...
//                + testIndex + "\">"
    ...
    private void titleResultRow(String label, int cq, String id) {
    	String color = "";
    	out.print("<tr");
    	if (id != null) {
    		out.print(" id=\"" + id + "\"");
    	...
    	out.println("><th class=\""+color+"\" colspan=\"" + cq + "\">" + label + "</th></tr>");
    	row = 0;
    }
    ...
    protected void startHtml(PrintWriter out) {
        ...
        out.println(".passedheader {background-color: #0A0}");
        out.println(".passedeven td {background-color: #3F3}");
        out.println(".skippedodd td {background-color: #CCC}");
        out.println(".skippedodd td {background-color: #DDD}");
        //out.println(".failedodd td,.numi_attn {background-color: #ff0000}");
        out.println(".numi_attn, .failedheader {background-color: #ff0000}");
        out.println(".failedeven td,.stripe .numi_attn {background-color: #ff0000}");
